...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case 'list_products': {
      ...
      return products.map((p) => ({ ...p, displayName: p.customerName || p.name }));
    }
    case 'get_product': {
      ...
      return { ...product, displayName: product.customerName || product.name };
    }
    case 'create_product': {
    ...
    case 'update_product': {
      const data = {};
      for (const key of ['name', 'customerName', 'sku', 'price', 'minPrice', 'categoryId', 'presentationText', 'adMatchTokens', 'companionProductIds', 'supplierId', 'supplierArticle', 'thumbnailUrl', 'images', 'aiNotes', 'sizeChartData', 'bulkPricing', 'isSet']) {
