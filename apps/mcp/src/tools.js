...
const TOOLS = [
  ...
  { name: 'create_category', description: 'Створити категорію.', inputSchema: { type: 'object', properties: { tenantId: { type: 'string' }, name: { type: 'string' }, description: { type: 'string' }, aiInstructions: { type: 'string', description: 'Що бот має дізнатись у клієнта для товарів цієї категорії (вільний текст)' }, requiredParams: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, unit: { type: 'string' }, hint: { type: 'string' } } }, description: 'Структурований список параметрів для збору перед оформленням (напр. [{name:"зріст",unit:"см"},{name:"вага",unit:"кг"}] для одягу, [{name:"розмір стопи",unit:"см"}] для взуття)' }, synonyms: { type: 'array', items: { type: 'string' }, description: 'Додаткові слова/синоніми, якими клієнт може назвати цю категорію в тексті (не прості відміни name, воронка стемить name сама) — напр. для категорії "Взуття": ["лофери","кросівки","туфлі","черевики","кеди"]; для "Куртки": ["вітровка"]. Воронка звіряє ці слова з повідомленням клієнта, щоб розпізнати намір і не показати не той товар.' } }, required: ['tenantId', 'name'] } },
  { name: 'update_category', description: 'Оновити категорію.', inputSchema: { type: 'object', properties: { categoryId: { type: 'string' }, name: { type: 'string' }, description: { type: 'string' }, aiInstructions: { type: 'string' }, requiredParams: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, unit: { type: 'string' }, hint: { type: 'string' } } } }, synonyms: { type: 'array', items: { type: 'string' } } }, required: ['categoryId'] } },
  ...
  { name: 'create_fop', description: 'Створити ФОП.', inputSchema: { type: 'object', properties: { tenantId: { type: 'string' }, name: { type: 'string' }, iban: { type: 'string' }, cardNumber: { type: 'string', description: 'Номер картки для card-to-card оплати (окрім IBAN)' }, taxId: { type: 'string', description: 'ІПН' }, monobankToken: { type: 'string' } }, required: ['tenantId', 'name'] } },
  { name: 'update_fop', description: 'Оновити ФОП. isActive:true робить його єдиним активним для tenant (деактивує інші).', inputSchema: { type: 'object', properties: { fopId: { type: 'string' }, name: { type: 'string' }, iban: { type: 'string' }, cardNumber: { type: 'string' }, taxId: { type: 'string' }, monobankToken: { type: 'string' }, isActive: { type: 'boolean' } }, required: ['fopId'] } },
  ...
  { name: 'update_set_components', description: 'Задати склад набору {componentProductId,qty,fixedColor?}[] — повна заміна.', inputSchema: { type: 'object', properties: { productId: { type: 'string' }, components: { type: 'array', items: { type: 'object', properties: { componentProductId: { type: 'string' }, qty: { type: 'number' }, fixedColor: { type: 'string', description: 'Колір цієї позиції, зафіксований САМЕ в межах цього набору (напр. джинси завжди сині в цьому комплекті) — воронка більше не питає клієнта і не вгадує; порожньо/не задано = як раніше' } } } } }, required: ['productId', 'components'] } },
...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case 'create_category':
      if (!args.name) throw new ValidationError('name обовʼязкове');
      return db.category.create({ data: { tenantId: args.tenantId, name: args.name, description: args.description || null, aiInstructions: args.aiInstructions || null, requiredParams: Array.isArray(args.requiredParams) ? args.requiredParams : [], synonyms: Array.isArray(args.synonyms) ? args.synonyms : [] } });
    case 'update_category':
      return db.category.update({
        where: { id: args.categoryId },
        data: { ...(args.name !== undefined ? { name: args.name } : {}), ...(args.description !== undefined ? { description: args.description } : {}), ...(args.aiInstructions !== undefined ? { aiInstructions: args.aiInstructions } : {}), ...(args.requiredParams !== undefined ? { requiredParams: Array.isArray(args.requiredParams) ? args.requiredParams : [] } : {}), ...(args.synonyms !== undefined ? { synonyms: Array.isArray(args.synonyms) ? args.synonyms : [] } : {}) },
      });
    ...
    case 'create_fop':
      if (!args.name) throw new ValidationError('name обовʼязкове');
      return db.fop.create({ data: { tenantId: args.tenantId, name: args.name, iban: args.iban || null, cardNumber: args.cardNumber || null, taxId: args.taxId || null, monobankToken: args.monobankToken || null } });
    case 'update_fop': {
      ...
      const data = {};
      for (const key of ['name', 'iban', 'cardNumber', 'taxId', 'monobankToken']) if (args[key] !== undefined) data[key] = args[key];
    ...
    case 'list_products': {
      ...
      const products = await db.product.findMany({ where, include: { offers: true, category: true, supplier: true }, orderBy: { updatedAt: 'desc' } });
      return products.map(withOfferAvailability);
    }
    case 'get_product': {
      ...
      return withOfferAvailability(product);
    }
    case 'create_product': {
      ...
      return db.product.create({
        data: {
          tenantId: args.tenantId, name: args.name, customerName: args.customerName || null, sku: args.sku, price: args.price, minPrice: args.minPrice ?? null,
    ...
    case 'update_set_components': {
      await db.$transaction([
        ...
        ...(args.components.length ? [db.productSetComponent.createMany({ data: args.components.map((c) => ({ parentProductId: args.productId, componentProductId: c.componentProductId, qty: Number(c.qty) || 1, fixedColor: c.fixedColor ? String(c.fixedColor).trim() : null })) })] : []),
      ]);
